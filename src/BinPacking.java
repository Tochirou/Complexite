...
public class BinPacking {
	...
	public void customSort(){
		...
		List<Obj> test = new ArrayList<Obj>();
		List<Obj> test2 = new ArrayList<Obj>();
		for(Obj o:sortedObjects) {
			if (o.getHeight() < o.getWidth()) test.add(o);
			else test2.add(o);
		}
		...
		while (!test.isEmpty() || !test2.isEmpty()) {
			if (!test2.isEmpty()) {
				sortedObjects.add(test2.get(0));
				test2.remove(0);
			} if (!test.isEmpty()) {
				sortedObjects.add(test.get(0));
				test.remove(0);	
			}
		}
	...
	private ArrayList<Method>getSortMethods(){
		...
		try {
			...
			sortMethods.add(BinPacking.class.getMethod("customSort", null));
